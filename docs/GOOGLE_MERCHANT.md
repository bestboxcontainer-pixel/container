...
| `tax` | Réservé aux États-Unis. En Allemagne la TVA est incluse dans `price`. |
...
> 30 derniers jours**. Un barré fictif est une infraction en Allemagne, et Google le traite
...
- `hasMerchantReturnPolicy` : 30 jours, `ReturnByMail`, Allemagne.
...
2. **Revendiquer le domaine** `bestboxcontainer.de` (Search Console ou balise HTML).
...
6. **Ajouter la source de données** : *Produktdatenquellen → Datenquelle hinzufügen →
   ...
   - `https://bestboxcontainer.de/feed/google` (XML, recommandé)
   - `https://bestboxcontainer.de/feed/google-csv` (TSV)
...
7. **La politique de retour réelle.** `MERCHANT_RETURN_POLICY` reprend les 30 jours
   ...
   page Widerrufsbelehrung conforme au droit allemand.
...
NEXT_PUBLIC_SITE_URL="https://bestboxcontainer.de"
```
