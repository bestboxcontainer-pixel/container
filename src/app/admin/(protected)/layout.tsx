...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [counts, quoteCounts] = await Promise.all([countReviewsByStatus(), countQuoteRequestsByStatus()]);

  return (
    <div className="min-h-screen bg-muted lg:flex">
      <AdminSidebar email={session.email} pendingReviews={counts.pending} pendingQuotes={quoteCounts.new} />
